...
const GENERATE_PATH = '/v1/flux-pro-1.1';
const KONTEXT_PATH = '/v1/flux-kontext-pro';
...
async function generateLookImageFlux(prompt, opts) {
  ...
  var payload = JSON.stringify({
    prompt: prompt,
    /* Макс. 1440 у flux-pro-1.1 (см. openapi.json) — раньше был 1024×1536
       у YandexART, тут 960×1440 сохраняет тот же портретный кадр 2:3. */
    width: opts.width || 960,
    height: opts.height || 1440,
    output_format: 'jpeg',
...
/* Правка по фото (FLUX.1 Kontext) — в отличие от generateLookImageFlux()
   (рисует картинку с нуля по тексту, без опоры на реальное фото), эта
   функция передаёт САМО фото клиентки в input_image и просит поменять
   только одежду — так сохраняются реальные лицо, телосложение, поза и фон,
   а не "воображаемая" моделью фигура. Контракт на выходе тот же: base64
   JPEG без data:-префикса.

   2026-09-19 — добавлено после того, как чистая text-to-image генерация
   (что через YandexART, что через generateLookImageFlux) регулярно рисовала
   не то телосложение и не те цвета вещей — у неё физически нет доступа к
   фото, только к тексту-описанию. */
async function generateLookImageKontext(prompt, photoBase64, opts) {
  opts = opts || {};
  var apiKey = process.env.FLUX_API_KEY;
  if (!apiKey) {
    throw new Error('Flux: FLUX_API_KEY не задан в переменных окружения');
  ...
  if (!photoBase64) {
    throw new Error('Flux Kontext: не передано фото (photoBase64 пусой)');
  }
  ...
  var payload = JSON.stringify({
    ...
    output_format: 'jpeg',
    safety_tolerance: opts.safetyTolerance || 4,
    /* 2026-09-19 — первый живой тест показал слишком слабую правку (модель
       почти не меняла одежду, только слегка сместила цвет) — судя по
       документации BFL, upsampling помогает модели точнее интерпретировать
       промпт из нескольких пунктов правки за один вызов. */
  ...
  var submitRes = await httpsJson(API_HOST, 'POST', KONTEXT_PATH, headers, payload);
  var pollingUrl = submitRes.body && submitRes.body.polling_url;
  if (submitRes.status !== 200 || !pollingUrl) {
    throw new Error('Flux Kontext: не удалось поставить задачу на редактирование: ' + submitRes.status + ' ' + JSON.stringify(submitRes.body));
  }
  ...
  while (Date.now() < deadline) {
    ...
    if (status === 'Ready') {
      var sampleUrl = pollRes.body.result && pollRes.body.result.sample;
      if (!sampleUrl) {
        throw new Error('Flux Kontext: статус Ready, но result.sample отсутствует: ' + JSON.stringify(pollRes.body));
      }
      return await fetchImageAsBase64(sampleUrl);
    }
    if (status === 'Error' || status === 'Failed') {
      throw new Error('Flux Kontext: генерация завершилась ошибкой: ' + JSON.stringify(pollRes.body));
    }
  }

  throw new Error('Flux Kontext: не дождались результата редактирования за ' + POLL_TIMEOUT_MS + ' мс');
}

module.exports = {
  generateLookImageFlux: generateLookImageFlux,
  generateLookImageKontext: generateLookImageKontext
};
