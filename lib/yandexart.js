...
/* Собирает промпт для картинки образа по слоям (порядок значения не важен —
   ...
   "без текста на картинке" в конце промпта, а не сам длинный слой. 50 (а не
   32) с запасом укладывается в общий лимит даже при всех 5 незапропущенных
   слоях + fit — см. расчёт в PROMPT_MAX_LENGTH выше. */
function buildLookImagePrompt(layers, fit, skipKeys) {
  ...
  var parts = Object.keys(layers || {})
    ...
    .map(function (k) { return truncate(stripItemWrapper(layers[k]), 50); })
    .filter(Boolean);
  var prompt = 'Fashion-лукбук, образ в полный рост: ' + parts.join(', ') + '.';
  if (fit) {
