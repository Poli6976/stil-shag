...
function runGeneration(prompt, opts) {
  ...
  /* yandex-art-2.0, не yandex-art — Yandex переименовал модель в каталоге
     (см. AI Studio → Model Gallery → Модели → YandexART 2.0, URI инстанса).
     Старое имя без версии перестало существовать между 2026-08-28 и
     2026-09-10, из-за чего запросы падали с "Access to model
     art://.../yandex-art/latest denied" (код 7) — не ошибка прав/биллинга,
     несмотря на то что текст похож именно на это. */
  var payload = JSON.stringify({
    modelUri: 'art://' + folderId + '/yandex-art-2.0/latest',
    messages: [{ text: prompt.slice(0, PROMPT_MAX_LENGTH), weight: 1 }],
    generationOptions: {
      mimeType: 'image/jpeg',
      /* 2:3, не 1:1 — квадратный кадр физически не оставляет места нарисовать
         фигуру в полный рост с обувью, модель вынуждена обрезать по бёдра/
         колено независимо от текста промпта. Портретный кадр даёт вертикальный
         запас под голову-плечи-ноги-обувь одновременно. */
      aspectRatio: { widthRatio: opts.widthRatio || 2, heightRatio: opts.heightRatio || 3 }
    }
  });
  var headers = {
...
/* Опрашивает операцию, пока не done. 45 попыток × 2с = до 90с — с запасом под
   yandex-art-2.0 (см. комментарий у modelUri выше): после перехода на неё с
   версии без номера 15 попыток (30с) стало не хватать, запрос падал с "не
   дождались результата генерации" (2026-09-10). Укладывается в лимит функции
   даже на Vercel Hobby — с включённым Fluid Compute там 300с и по умолчанию,
   и максимум (проверено 2026-08-26 в дашборде проекта и в актуальной
   документации Vercel). */
