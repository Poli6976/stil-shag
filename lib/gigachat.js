...
/* fileId необязателен — без него это обычный текстовый чат без вложения. Можно передать один id
   (строка) или несколько сразу (массив, например для сравнения двух картинок в одном запросе —
   см. imageNeedsRetry в api/compose-look.js). */
function chatWithImage(token, systemPrompt, userText, fileId, opts) {
  opts = opts || {};
  var attachments = fileId ? (Array.isArray(fileId) ? fileId : [fileId]) : undefined;
  var payload = JSON.stringify({
    model: opts.model || process.env.GIGACHAT_MODEL || 'GigaChat-2-Max',
    temperature: opts.temperature != null ? opts.temperature : 0.3,
    messages: [
      { role: 'system', content: systemPrompt },
      { role: 'user', content: userText, attachments: attachments }
    ]
  });
