...
/* Атомарно гасит бесплатный первый образ пользователя. Возвращает true, если
   именно этот вызов его погасил (образ бесплатный), false — если он уже был
   использован раньше (нужно платить/списывать скидку). */
async function consumeFreeLook(userId) {
  var supabase = getSupabaseAdmin();
  var result = await supabase.rpc('consume_free_look', { p_user_id: userId });
  if (result.error) throw result.error;
  return !!result.data;
}
...
module.exports = {
  ...
  consumeFreeLook: consumeFreeLook
};
