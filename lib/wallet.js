...
async function getBalance(userId) {
  ...
  var result = await supabase
    .from('wallets')
    .select('balance_kopecks, currency, free_credits_remaining')
    .eq('user_id', userId)
    .single();
  if (result.error) throw result.error;
  return {
    balanceKopecks: result.data.balance_kopecks,
    currency: result.data.currency,
    freeCreditsRemaining: result.data.free_credits_remaining
  };
}
...
/* Плоское списание без бонусов пунш-карты (не начисляет free_credits_remaining
   и не трогает discount_credits) — для дешёвых продуктов вроде «Образа по
   фото», см. комментарий 2026-09-13 в db/schema.sql у debit_wallet_flat. */
async function debitFlat(userId, packageCode, priceKopecks) {
  var supabase = getSupabaseAdmin();
  var result = await supabase.rpc('debit_wallet_flat', {
    p_user_id: userId,
    p_package_code: packageCode,
    p_price_kopecks: priceKopecks
  });
...
/* Атомарно проверяет и занимает один бесплатный образ, начисленный прошлой
   оплатой (пунш-карта, см. lib/lookAccess.js — 2 бесплатных за полную цену,
   1 за оплату по коду партнёра). Возвращает true, если слот был и его занял
   именно этот вызов, false — если бесплатных образов не осталось (нужно
   платить/списывать скидку). */
